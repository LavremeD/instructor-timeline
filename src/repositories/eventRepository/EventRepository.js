...
class EventRepository{
    ...
    async listEventById({event_id}){
        const event = await knex("events").where({event_id})
        return event
    }
    async updateEvent({event_id, period}){
        ...
        await knex("events").where({event_id}).update({period: event.period})

        return event
    }
    async deleteEvent({event_id}){
        const event = await knex("events").where({event_id}).delete()
        return event
    }
}
